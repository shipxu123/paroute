...
// void CirDB::setPAGuide(const Int_t horGuide, const Int_t verGuide, const Int_t viaGuide) {
//   _horGuide = horGuide;
//   _verGuide = verGuide;
//   _viaGuide = viaGuide;
// }

void CirDB::setPAGuide(Vector_t<Int_t> horGuide, Vector_t<Int_t> verGuide, Vector_t<Int_t> viaGuide) {
  _horGuide = horGuide;
  _verGuide = verGuide;
  _viaGuide = viaGuide;
}

void CirDB::setPADrCritical(Vector_t<Int_t> v_critical) {
  Int_t signal_idx = 0;
  Int_t critical_idx = 0;

  for (Int_t i = 0; i < (Int_t)this->numNets(); ++i) {
    Net* pNet = &this->net(i);
    if (!pNet->bPower())
    {
      if (critical_idx < v_critical.size() && signal_idx++ == v_critical[critical_idx])
      {
        pNet->setDrCritical(v_critical[critical_idx++]);
      }
    }
  }
}
