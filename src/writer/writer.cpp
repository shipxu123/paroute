...
// PA Dumb File
void Writer::writePADumb(const String_t& placementFilename, const String_t& outputFilename) {
  // fprintf(stdout, "Writing PA Dumb File %s\n", outputFilename.c_str());
  // _padumbw.write(placementFilename, outputFilename);
  String_t pinFilename = outputFilename + ".pin";
  fprintf(stderr, "Writing PARoute PIN File %s\n", pinFilename.c_str());
  FILE* fout = fopen(pinFilename.c_str(), "w");
  fprintf(fout, "num net %d\n", (Int_t)_cir.numNets());
  for (Int_t i = 0 ; i < (Int_t)_cir.numNets(); ++i) {
    const Net& net = _cir.net(i);
    fprintf(fout, "%s %d %d\n", net.name().c_str(), i, (Int_t)net.numPins());
  ...
  String_t netFilename = outputFilename + ".net";
  fprintf(stdout, "Writing PA Dumb Net File %s\n", netFilename.c_str());
  _padumbw.write(placementFilename, netFilename);
}
